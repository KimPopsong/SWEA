	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 1; tc <= testCase; tc++) {
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < 8; j++) {
					cog.get(i).add(Integer.parseInt(st.nextToken()));
				}
			}
			...
			for (int m = 0; m < moveNumber; m++) {
				...
				for (int right = moveCog + 1; right < 4; right++) { // 기준 톱니바퀴의 오른쪽부터 확인
					int magnet;

					if (tempDirection == 1) { // 이전의 톱니바퀴가 시계 방향으로 회전했다면
					...
					if (magnet != cog.get(right).get(6)) { // 붙어있는 극의 극성이 다르다면
						if (tempDirection == 1) { // 이전 톱니바퀴가 시계 방향으로 회전했다면
							tempDirection = -1;
				...
				for (int left = moveCog - 1; left >= 0; left--) { // 기준 톱니바퀴의 왼쪽 확인
					int magnet;
					...
					if (magnet != cog.get(left).get(2)) { // 붙어있는 극의 극성이 다르다면
						if (tempDirection == 1) { // 이전 톱니바퀴가 시계 방향으로 회전했다면
							tempDirection = -1;
